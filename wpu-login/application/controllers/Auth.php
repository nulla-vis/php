...
class Auth extends CI_Controller
{
    ...
    public function registration()
    {
        ...
        $this->form_validation->set_rules('password1', 'Password', 'required|trim|min_length[3]|matches[password2]', [
            'matches' => 'Password doesn\'t mathes',
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_port' => 465,
            'smtp_user' => 'appact041@gmail.com',
            'smtp_pass' => 'usemahuA41',
            'mailtype'  => 'html',
            'charset'   => 'utf-8',
            'newline'   => "\r\n",
        ];
        ...
        $this->email->from('appact041@gmail.com', 'WPU-Login Application');
        ...
        if ($user) {
            ...
            if ($user_token) {
                if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
                    ...
                    $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
                    Account activation , token expired</div>');
                    redirect('auth');
                }
            } else {
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
                Account activation , token invalid</div>');
                redirect('auth');
            }
        } else {
            $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
            Account activation , email invalid</div>');
            redirect('auth');
        }
    }
    ...
    public function forgotPassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($user) {
                ...
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
                Email is not registered not activated</div>');
                redirect('auth/forgotpassword');
            }
        }
    }
